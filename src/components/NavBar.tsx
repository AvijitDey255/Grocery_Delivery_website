...
const NavBar = ({ user }: { user: IUser }) => {
  return (
    <div className="w-[95%] fixed top-4 left-1/2 -translate-x-1/2 bg-linear-to-r from-green-500 to-green-700 h-16 rounded-2xl shadow-lg shadow-black/30 flex justify-between items-center px-4 md:px-8 z-50">
      <Link
        href={"/"}
        className="text-white font-semibold text-2xl sm:text-3xl  tracking-wide hover:scale-105 transition-transform"
      >
        Snapcart
      </Link>
      ...
      <div className="flex gap-3 items-center md:gap-6 relative">
        <Link
          href={""}
